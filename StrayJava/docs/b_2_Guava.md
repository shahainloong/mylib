...
在23.1（包含23.1）向后的的高版本细分为jre和android版本，相应的version是：
...
参考：http://www.ehcache.org/documentation/3.5/cache-event-listeners.html

Ehcache提供了一个抽象类CacheEventAdapter，这个类实现了事件监听，重写里面的事件方法就可以监听对应的事件了，这样就不用自己去实现事件监听了。
